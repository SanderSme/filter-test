...
function show(postData) {
  ...
  myFilter.onkeyup = function (event) {
    ...
    for (let i = 0; i < filteredPosts.length; i++) {
      ...
      blogList.innerHTML += `<div class="blog-post"><img src="${
        filteredPosts[i].acf.hero_image.url
      }" alt="${filteredPosts[i].acf.hero_image.alt}" class="blog-post-img" />
          <div class="blog-info-container">
            <h2 class="post-heading post-info">${filteredTitle}</h2>
            <h3 class="category post-info">${filteredPosts[i].acf.category}</h3>
            <p class="post-date post-info">Posted on: ${
              filteredPosts[i].date.split("T")[0]
            }</p>
            <p class="summary">
